...
const getLink = async (page) => {
    ...
    let logSKPDSudah = []
    let urut = 1
    let logFileSudah = `${PATH.DPA.JSON}\\logSKPDSudah.json`;
    if(fs.existsSync(logFileSudah)){
        let jsonContent = fs.readFileSync(logFileSudah);
        logSKPDSudah = JSON.parse(jsonContent);
    }
    for (const [index, skpd] of listSKPD.entries()) {
        ...
        if(logSKPDSudah.length !== 0){
            if(logSKPDSudah[index].nama_skpd === namaSKPD){
                listSKPDSudah.push(logSKPDSudah[index])
                urut = parseInt(logSKPDSudah[index].urut)
                continue;
            }
        }
        ...
        if(fs.existsSync(logFileSudah)){
            fs.rmSync(logFileSudah);
        }

        fs.writeFile(logFileSudah, JSON.stringify(listSKPDSudah), function(err) { 
            if (err) {
                console.log('File Log tidak bisa disimpan', err)
            }
            console.log('Log File SKPD yang sudah diambil kegiatannya berhasil disimpan')
        });
