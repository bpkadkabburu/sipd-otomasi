...
const getLink = async (page) => {
    ...
    for (const [index, skpd] of listSKPD.entries()) {
        ...
        let skpdJSON = `${rbFolder}\\${namaSKPD}.json`
        if(fs.existsSync(skpdJSON)){
            console.log(`JSON ${namaSKPD} sudah ada, melanjutkan ...`)
            if(namaSKPD === '26. Sekretariat Daerah'){
                urut += 9
            }else if(namaSKPD === '1. Dinas Pendidikan dan Kebudayaan'){
                urut += 2
            } else {
                urut++
            }
        ...
        // komen diatas tidak berlaku, kita uji coba tidak perlu di cek apakah dia ada rinciannya
        ...
        rincianBelanjaSKPD = _.sortBy(rincianBelanjaSKPD, "kode_sub_skpd")
        ...
        console.log(`Menyimpan SKPD ${namaSKPD} DPA Rincian Belanja .....`)
        fs.writeFile(`${rbFolder}\\${namaSKPD}.json`, JSON.stringify(listCetakPerSKPD), function(err) { 
            if (err) {
                console.log('File JSON tidak bisa disimpan', err)
            }
            console.log(`JSON SKPD ${namaSKPD} Berhasil Disimpan`)
        });
    }
}
...
const retryDownload = async (errorList, destination) => {
    let errorJson = `${destination}\\error.json`
    let errorSolve = 0
    for (const errKegiatan of errorList) {
        let name = errKegiatan.kegiatan.substring(0,100)
        ...
        do {
            try {
                await d(errKegiatan.link, destination, dOptions);
                console.log(`file tersimpan di ${pathFile}`)  
                success = 1
            } catch (error) {
                console.log(error)
                count++
    ...
    if(errorSolve === errorList.length){
        if(fs.existsSync(errorJson)){
            fs.rmSync(errorJson)
        }
    }
